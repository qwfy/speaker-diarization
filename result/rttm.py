...
def convert_tsv_to_rttm(in_file_path, out_file_path):
  ...
  rows = [
    Rttm(basename, start, stop-start, speaker)
    for start, stop, speaker in lines
  ]
  ...
  with open(in_file_path) as f:
    for line in f:
      ...
      if line:
        ...
        lines.append(Rttm(file_id, float(start), float(duration), speaker))
  return lines
...
def merge_rttm_by_adjacency(in_file_path, out_file_path):
  ...
  # drop index
  merged = [
    row for _index, row in new_rows
  ]
...
def rttm_to_tsv(in_file_path, out_file_path):
  ...
  rows = [
    (rttm.start, rttm.stop, rttm.speaker)
    for rttm in rttms
  ]
      ...
      for i2, row2 in rows_with_index:
        ...
        if i2 != i and row2.speaker == row.speaker:
          ...
          if stop1 >= start2:
            ...
            if (
                not is_on_the_other_speaker(stop1, rows_with_index, i, i2, row.speaker)
                and not is_on_the_other_speaker(start2, rows_with_index, i, i2, row.speaker)
                and not is_on_the_other_speaker((min(stop1, start2), max(stop1, start2)), rows_with_index, i, i2, row.speaker)
            ):
              merge = True
          else:
            pass
...
def is_on_the_other_speaker(end, rows, r1, r2, speaker):
  new_rows = [
    row
    for i, row in rows
    if i != r1 and i != r2 and row.speaker != speaker
  ]
  for row in new_rows:
...
def merge_rttm_by_st(in_file_path, out_file_path):
  ...
  all_rows.sort()
  all_rows = [
    Rttm(file_id, start, stop-start, speaker)
    for start, stop, speaker in all_rows
  ]
...
def plot_st(in_rttm_paths, save_path, hints=[]):
  rowss = [
    [[row.start, row.stop, row.speaker]
     for row in load_rttm(in_rttm_path)
     ]
